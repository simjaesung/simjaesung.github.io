...
관련하여 많은 것들이 있지만, 이번 글에서는 `enum`과 `as const`에 대해서 다뤄보려 합니다.
...
이를 따르다 보면, 자연스럽게 이 두 가지 방식 중 하나를 선택해야 하는 상황을 맞이하게 됩니다.

<br/>

하지만 돌이켜보면 `enum` 과 `as const` 중 하나를 선택하는 과정에서 명확한 근거없이 기분에 따라 사용을 했던 것 같습니다. (부끄럽네요..)

그래서 이 둘 각각의 특징을 알아보며 스스로 컨벤션을 정해보려합니다.
